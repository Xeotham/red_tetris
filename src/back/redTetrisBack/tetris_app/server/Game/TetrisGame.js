...
const S_1 = require("./Pieces/S");
const T_1 = require("./Pieces/T");
const Z_1 = require("./Pieces/Z");
const L_1 = require("./Pieces/L");
const J_1 = require("./Pieces/J");
const O_1 = require("./Pieces/O");
const I_1 = require("./Pieces/I");
...
class TetrisGame {
    ...
    constructor(player, username = null) {
        ...
        this.size = new Pos.Pos(tc.TETRIS_WIDTH, tc.TETRIS_HEIGHT);
        this.matrix = new Matrix.Matrix(this.size.add(0, tc.BUFFER_HEIGHT));
    ...
    shuffleBag() {
        const pieces = [
            new S_1.S(this.rotationType),
            new T_1.T(this.rotationType),
            new Z_1.Z(this.rotationType),
            new L_1.L(this.rotationType),
            new J_1.J(this.rotationType),
            new O_1.O(this.rotationType),
            new I_1.I(this.rotationType)
        ];
    ...
    async spawnPiece() {
        ...
        if (!this.currentPiece)
            return;
        this.currentPiece.setCoordinates(new Pos.Pos(3 - 2, tc.BUFFER_HEIGHT - 3 - 2)); // -2 to take piece inner size into account
        ...
        await (0, utils_1.delay)(this.spawnARE);
    }
    ...
    move(direction) {
        if (!this.currentPiece)
            return;
        ++this.keysPressed;
        const offset = direction === "left" ? new Pos.Pos(-1, 0) : new Pos.Pos(1, 0);
    ...
    async retry() {
        ...
        const restoredState = JSON.parse(JSON.stringify(this.initialState));
