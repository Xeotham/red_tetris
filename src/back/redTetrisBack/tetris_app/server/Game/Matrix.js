...
class Matrix {
    ...
    constructor(arg) {
        ...
        if (arg instanceof Matrix)
            this.matrix = arg.matrix.map((row) => row.map((mino) => new Mino.Mino(mino.getTexture(), mino.isSolid())));
    }
    ...
    createEmptyMatrix() {
        ...
        for (let y = 0; y < this.size.getY(); y++) {
            for (let x = 0; x < this.size.getX(); x++)
                matrix[y].push(new Mino.Mino("EMPTY"));
            matrix.push([]);
        }
        return matrix;
    }
    ...
    at(arg1, arg2) {
        let pos;
        if (arg1 instanceof Pos.Pos)
            pos = arg1;
        else
            pos = new Pos.Pos(arg1, arg2);
        if (!pos.equals(pos.clamp(new Pos.Pos(0, 0), this.size.subtract(1, 1))))
            return Matrix.full;
    ...
    setAt(arg1, arg2, arg3) {
        ...
        if (!pos.equals(pos.clamp(new Pos.Pos(0, 0), this.size.subtract(1, 1))))
            return;
        this.matrix[pos.getY()][pos.getX()] = arg1 instanceof Pos.Pos ? arg2 : arg3;
    }

    isMinoAt(arg1, arg2) {
        let pos;
        if (arg1 instanceof Pos.Pos)
            pos = new Pos.Pos(arg1);
        else
            pos = new Pos.Pos(arg1, arg2);
        ...
        for (let y = 0; y < this.size.getY(); ++y) {
            if (this.matrix[y][0].getShouldRemove()) {
                for (let x = 0; x < this.size.getX(); ++x) {
                    ...
                    this.matrix[0][x] = new Mino.Mino("EMPTY");
                }
                ++lineRemoved;
            }
        }
    ...
    addGarbage(lines) {
        ...
        for (let y = this.size.getY() - lines; y < this.size.getY(); ++y) {
            for (let x = 0; x < this.size.getX(); ++x) {
                if (x === hole)
                    this.matrix[y][x] = new Mino.Mino("EMPTY");
                else
                    this.matrix[y][x] = new Mino.Mino("GARBAGE", true);
            }
        }
        return "";
    }
...
Matrix.full = new Mino.Mino("Full", true);
