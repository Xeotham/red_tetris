...
class ATetrimino {
    ...
    constructor(rotationType, name = "None", coordinates = new Pos.Pos(0, 0), texture = "Empty") {
        this.name = name;
    ...
    static convertBlock(jsonBlock) {
        ...
        for (let i = 0; i < jsonBlock.blocks?.length || 0; ++i)
            blocks.push(new Pos.Pos(jsonBlock.blocks[i].x, jsonBlock.blocks[i].y));
        let original = [];
        for (let i = 0; i < jsonBlock.original?.length || 0; ++i)
            original.push(new Pos.Pos(jsonBlock.blocks[i].x, jsonBlock.blocks[i].y));
        let SRS = [];
        for (let i = 0; i < jsonBlock.SRS?.length || 0; ++i)
            SRS.push(new Pos.Pos(jsonBlock.SRS[i].x, jsonBlock.SRS[i].y));
        let SRSX = [];
        for (let i = 0; i < jsonBlock.SRSX?.length || 0; ++i)
            SRSX.push(new Pos.Pos(jsonBlock.SRSX[i].x, jsonBlock.SRSX[i].y));
        return ({
            blocks: blocks,
            original: original,
            SRS: SRS,
    ...
    rotate(direction, matrix) {
        ...
        for (let i = 0; i < startRotations.length; ++i) {
            const startPos = (direction === "180" ? new Pos.Pos(3, 3) : startRotations[i]);
    ...
    isColliding(matrix, offset = new Pos.Pos(0, 0)) {
        const struct = this.getStruct();
    ...
    place(matrix, isSolid = false, isShadow = false) {
        ...
        for (let i = 0; i < struct.nbBlocks; ++i) {
            ...
            if (!isShadow || (isShadow && matrix.at(pos).isEmpty())) {
                matrix.setAt(pos, new Mino.Mino(this.texture, isSolid));
                if (isShadow)
    ...
    remove(matrix, isShadow = false) {
        ...
        for (let i = 0; i < struct.nbBlocks; ++i) {
            ...
            if (!isShadow || (isShadow && matrix.at(pos).getIsShadow()))
                matrix.setAt(pos, new Mino.Mino("EMPTY", false));
        }
    }
    ...
    canFall(matrix) {
        return !this.isColliding(matrix, new Pos.Pos(0, 1));
    }

    canSlide(matrix) {
        return !this.isColliding(matrix, new Pos.Pos(1, 0)) || !this.isColliding(matrix, new Pos.Pos(-1, 0));
    }
}
