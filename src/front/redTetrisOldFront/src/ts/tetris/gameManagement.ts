...
import { postToApi, address, getFromApi } from "../utils.ts";
import { tetrisBoardHtml } from "./tetrisHTML.ts";
import { imTexts } from "../imTexts/imTexts.ts";
import { userKeys } from "./utils.ts";
import { TCS } from "../TCS.ts";
import { zoneGame } from "../immanence.ts";
// @ts-ignore
...
export let username: string = generateInviteCode();
...
const socketClose = async (type: string) => {
	try {
		...
		if (type === "unload")
			postToApi(`http://${address}/api/user/disconnect-user`, {}).catch();
...
export const    searchGame = () => {
	socket = new WebSocket(`ws://${address}/api/tetris/matchmaking?username=${username}`);
	socketInit(socket);
}
...
export const	hideZoneGame = () => {
	if (zoneGame && !zoneGame.classList.contains("hidden"))
		zoneGame.classList.add("hidden");
	// TODO : Need zoneTetris?
	// if (zoneTetris && zoneTetris.classList.contains("hidden"))
	// 	zoneTetris.classList.remove("hidden");
}

export const	showZoneGame = () => {
	if (zoneGame && zoneGame.classList.contains("hidden"))
		zoneGame.classList.remove("hidden");
	// if (zoneTetris && !zoneTetris.classList.contains("hidden"))
	// 	zoneTetris.classList.add("hidden");
}